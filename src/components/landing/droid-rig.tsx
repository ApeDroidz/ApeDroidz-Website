...
// Только белый дроид — тот, с которого лендинг начинался (владелец, 19.09:
// «на главной поставим только белого дроида, без сменяющихся»). Подборка
// коллекционных (#1, #620, #4, #77, #3273, #1752 из
// apedroidz/3D-media/hero/v2/<id>.glb, собранных из актуальных трейтов
// scripts/3d/bake_hero_droid.py) снята с показа, но риг остался общим: чтобы
// вернуть смену, достаточно дописать адреса в этот список — при одной модели
// подмена просто не запускается (HERO_MODELS.length > 1 ниже).
const HERO_MODELS = [
  "/white-droid.glb",
]
