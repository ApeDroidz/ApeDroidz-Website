    ...
    <div className="w-full h-full flex flex-col items-center justify-start pt-4 md:pt-4 p-0 relative overflow-hidden">
      ...
      <div className="relative w-full max-w-[1100px] flex flex-col items-center">
        <AnimatePresence mode="wait">
          {!showSuccessScreen ? (
            <motion.div key="upgrade-machine" initial={{ opacity: 0, y: 0 }} animate={{ opacity: 1, y: [-10, 10, -10], rotate: [-1, 1, -1] }} transition={{ y: { duration: 6, repeat: Infinity, ease: "easeInOut" }, rotate: { duration: 6, repeat: Infinity, ease: "easeInOut" } }} className="relative w-full sm:w-[85%] aspect-video flex items-center justify-center z-10" >
              <GlitchContainer intensity={glitchIntensity}>
                <div className="relative w-full h-full flex items-center justify-center">
                  ...
                  <div className="absolute z-20 flex items-center justify-center" style={{ top: '28%', left: '17.3%', width: '20.5%', height: '26%', mixBlendMode: 'screen', backgroundColor: 'black', transform: 'perspective(1500px) rotateY(30deg) rotateX(15deg) rotateZ(-5.7deg) skewX(1deg)', clipPath: 'inset(0 round 14px)' }}>
                    {selectedBattery ? <ScreenContent item={selectedBattery} rounded={true} /> : <span className="text-white/30 font-mono text-[10px] md:text-sm animate-pulse tracking-widest">BATTERY</span>}
                  </div>
                  <div className="absolute z-20 flex items-center justify-center" style={{ top: '16%', left: '50.8%', width: '26%', height: '33%', mixBlendMode: 'screen', backgroundColor: 'black', transform: 'perspective(1600px) rotateY(-31deg) rotateX(14deg) rotateZ(5deg) skewY(-2.6deg)', clipPath: 'inset(0 round 14px)' }}>
                    {selectedDroid ? <ScreenContent item={selectedDroid} showName={true} rounded={true} /> : <span className="text-white/30 font-mono text-[12px] md:text-sm animate-pulse tracking-widest">DROID</span>}
                  </div>
                </div>
              </GlitchContainer>
            </motion.div>
