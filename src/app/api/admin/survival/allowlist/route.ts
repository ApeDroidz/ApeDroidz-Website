...
import { DEFAULT_ACCESS_DURATION, expiryFor } from '@/lib/survivalDurations'

export const dynamic = 'force-dynamic'
const headers = { 'Cache-Control': 'no-store' }

/**
 * POST /api/admin/survival/allowlist { action: 'add' | 'revoke', wallet, note?, duration? }
 *
 ...
 * `revoked_at` is null and `expires_at` (if set) is in the future. Adding a revoked or an
 * expired wallet activates it again (original added_at kept, the note replaced only if a new
 * one is given) for the `duration` picked (lib/survivalDurations.ts; default: no expiry).
 */
export async function POST(request: NextRequest) {
    ...
    const duration = typeof body.duration === 'string' ? body.duration : DEFAULT_ACCESS_DURATION
    const expiry = expiryFor(duration)
    if (expiry === undefined) return NextResponse.json({ error: 'Unknown duration' }, { status: 400, headers })
    const expires_at = expiry ? expiry.toISOString() : null
    ...
    if (action === 'revoke') {
        ...
        const { error } = existing
            ? await supabaseAdmin.from('survival_allowlist').update({ revoked_at: null, expires_at, note: note ?? (existing as { note: string | null }).note }).eq('wallet', wallet)
            : await supabaseAdmin.from('survival_allowlist').insert({ wallet, note, added_by: 'spltpnl', expires_at })
        if (error) return NextResponse.json({ error: error.message }, { status: 500, headers })
    }
    logEvent({ level: 'info', kind: `allowlist.${action}`, wallet, message: note ?? '', data: action === 'add' ? { by: 'spltpnl', duration, expires_at } : { by: 'spltpnl' } })
    return NextResponse.json({ ok: true, wallet, action, expires_at: action === 'add' ? expires_at : undefined }, { headers })
}
