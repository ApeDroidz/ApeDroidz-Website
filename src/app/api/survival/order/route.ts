...
import { CASHIER, describe, encodePay, isSku, minWeiFor, SKUS } from '@/lib/survivalShop'
import { rpc } from '@/lib/survivalSettle'

/**
 * POST /api/survival/order { sku: 'run' | 'run10' | 'continue', runId?, platform?: 'site' | 'otherside' }
 ...
 *   → { ok: false, state: 'not_configured' | 'malformed' | 'no_season' | 'no_run' | 'rate_limited' }
 *
...
export async function POST(req: NextRequest) {
    ...
    if (sku === 'continue') {
        ...
        const q = supabaseAdmin.from('survival_runs').select('id, wallet, status').eq('wallet', caller.wallet).eq('status', 'started')
    ...
    const { data: order, error } = await supabaseAdmin.from('survival_orders').insert({
        ...
        min_wei: minWeiFor(s.priceApe).toString(), run_id: runId, platform, from_block: fromBlock,
    }).select('id').single()
    ...
    return NextResponse.json({
        ...
        data: encodePay(caller.wallet, order.id), description: describe(sku),
    }, { headers: noStore })
}
